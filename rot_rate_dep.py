...
for i in range(NUMBER_OF_STEPS):
    ...
    tmp_res = rs.Resonator(3, rs.Mirror(0, 1 / 1000000, 0.005), rs.Mirror(0.009999, 10 / 180 * np.pi, 0.010), rs.Mirror(length, 0, 0.400))
...
for i in range(NUMBER_OF_STEPS):
    ...
    tmp_res = rs.Resonator(3, rs.Mirror(0, 1 / 1000000, 0.005), rs.Mirror(0.009999, 10 / 180 * np.pi, 0.010), rs.Mirror(0.310, 0, radius))
    angle = tmp_res.realign() * 1000000
    if tmp_res.is_g_stable_sagittal() and tmp_res.is_g_stable_tangential():
        na[i] = tmp_res.waist_search(1064 * 10 ** -9)[0, 0, 2]
    else:
